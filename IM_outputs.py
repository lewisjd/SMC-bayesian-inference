# from Sam McCullum's IonMonger PDE code
...
def get_hysteresis(J, V, v_oc_forward, v_oc_reverse):
    ...
    if reverse_size == forward_size:
        ...
        return simpson(difference) * 0.012
    else:
        ...
        region_1 = simpson(difference) * 0.12
        region_2 = simpson(extra) * 0.12
...
def calculate_outputs(J, V):
    ...
    # values = np.array([round(j_sc[0], 4), 
    #                  round(v_oc_reverse_actual[0], 4), 
    #                  round(v_oc_forward_actual[0], 4), 
    #                  round(mp_r, 4), 
    #                  round(mp_f, 4),
    #                  DoH])
    # names = ["Jsc","Vocr","Vocf","PCEr","PCEf","DoH"]
    ...
    values = np.array([round(j_sc[0], 4), 
                     ...
                     round(mp_r, 4), 
                     round(mp_f, 4)])
    names = ["Jsc","Vocr","Vocf","PCEr","PCEf"]

    return pd.DataFrame({"variable": names,
                         "value": values})